...
def calculate_acc(preprocess_config2, model_config, model, vocoder, \
    ...
    for batchs in tqdm(loader):
        ...
        """
        V2C-Net's quick_eval set. If you wanna test all data, please delete the following code.
        """
        if counter_batch == 2:
            break
...
def assess_spk_emo(encoder_spk, encoder_emo, sampling_rate, samples_path,
    ...
    wav_reconstructions_batch, wav_predictions_batch, tags_batch, speakers_batch, emotions_batch, cofs_batch):
    ...
    eval_acc_rec_emo, eval_acc_pred_emo = acc_metric(emotions_ids, emotions_batch, \
        wav_reconstructions_utterance_embeds_emo, wav_predictions_utterance_embeds_emo, \
        ids2loc_map_emo, loc2ids_map_emo, centroids=None)
    # speaker
    eval_acc_rec_spk, eval_acc_pred_spk = acc_metric(speakers_ids, speakers_batch, \
        wav_reconstructions_utterance_embeds_spk, wav_predictions_utterance_embeds_spk, \
...
def evaluate(model, step, configs, logger=None, vocoder=None, encoder_spk=None, \
    ...
    _, _, acc_means_train_spk, acc_means_train_emo, avg_mcd_train = calculate_acc(preprocess_config2, model_config, \
        model, vocoder, encoder_spk, encoder_emo, loader_train_acconly, logger, sampling_rate=sampling_rate, samples_path=train_samples_path, \
        mcd_box_plain=mcd_box_plain, mcd_box_dtw=mcd_box_dtw, mcd_box_adv_dtw=mcd_box_adv_dtw, useGT=useGT)
    ...
    batch, output, acc_means_val_spk, acc_means_val_emo, avg_mcd_val = calculate_acc(preprocess_config2, \
        model_config, model, vocoder, encoder_spk, encoder_emo, loader_val, logger, sampling_rate=sampling_rate, samples_path=val_samples_path, \
        mcd_box_plain=mcd_box_plain, mcd_box_dtw=mcd_box_dtw, mcd_box_adv_dtw=mcd_box_adv_dtw, useGT=useGT)
