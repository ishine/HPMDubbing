...
def log(
    ...
    if losses is not None:
        if LM == 1:
            ...
            logger.add_scalar("Loss/pitch_loss", losses[3], step)
            logger.add_scalar("Loss/energy_loss", losses[4], step)
            logger.add_scalar("Loss/mel_MSE", losses[5], step)
            logger.add_scalar("Loss/mel_postnet_MSE", losses[6], step)
        elif LM == 3:
            logger.add_scalar("Loss/total_loss", losses[0], step)
            logger.add_scalar("Loss/mel_loss", losses[1], step)
            logger.add_scalar("Loss/mel_postnet_loss", losses[2], step)
...
def synth_multi_samples(targets, predictions, vocoder, model_config, preprocess_config):
    ...
    for i in range(len(predictions[0])):
        ...
        mel_prediction = predictions[1][i, :mel_len].detach().transpose(0, 1)
        # duration_sum = targets[11][i, :src_len].detach().cpu().numpy()  #
        duration_sum = targets[11][i].detach().cpu().numpy().item()
        # duration_prediction = predictions[5][i, :].detach().cpu().numpy()
        # duration_prediction = targets[11][i, :src_len].detach().cpu().numpy()
        duration_prediction_sum = targets[16][i].item()*4
        cofs.append((duration_sum, duration_prediction_sum))
        # cofs.append((duration.sum(), duration_prediction))
